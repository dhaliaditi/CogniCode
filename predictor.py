...
All five tools are used when available:
  Radon, Pylint, Prospector  — always run (no extra setup)
  SonarQube                  — optional, requires Docker
  Understand                 — optional, requires und binary
"""
...
from collectors.sonarqube_col  import SonarQubeCollector
from collectors.understand_col import UnderstandCollector
...
def predict_directory(
    ...
    sonar_host: str = "",
    sonar_user: str = "admin",
    sonar_password: str = "admin",
    sonar_token: str = "",
    sonar_project: str = "",
    und_bin: str = "",
) -> dict:
    ...
    if sonar_host:
        try:
            project_key = sonar_project or f"cognicode_{root.name}"
            sonar_rows = SonarQubeCollector(
                host=sonar_host,
                admin_user=sonar_user,
                admin_password=sonar_password,
                token=sonar_token,
                project_key=project_key,
            ).collect_batch(root)
            tools_used.append("SonarQube")
        except Exception as e:
            warnings.append(f"SonarQube failed: {e}")

    # Optional: Understand
    understand_rows: dict = {}
    und_by_basename: dict = {}
    if und_bin:
        try:
            understand_rows = UnderstandCollector(und_bin=und_bin).collect_batch(root)
            und_by_basename = {Path(k).name: v for k, v in understand_rows.items()}
            tools_used.append("Understand")
        except Exception as e:
            warnings.append(f"Understand failed: {e}")
...
def predict_file(file_path: Path, **kwargs) -> dict:
    return predict_directory(file_path.parent, single_file=file_path, **kwargs)


def predict_from_github(repo_url: str, **kwargs) -> dict:
    import subprocess
    ...
    try:
        ...
        return predict_directory(Path(tmp), **kwargs)
